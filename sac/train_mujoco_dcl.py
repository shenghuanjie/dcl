...
def train_SAC(env_name, exp_name, seed, logdir, two_qf=False, reparam=False):
    ...
    sampler2 = utils.SimpleSampler(**sampler_params)
    ...
    sampler2.initialize(env2, policy, replay_pool)
    ...
    with tf.Session(config=tf_config):
        ...
        for epoch in algorithm.train(sampler, n_epochs=250):
        ...
        for epoch in algorithm.train(sampler2, n_epochs=250):
            logz.log_tabular('Iteration', epoch)
            for k, v in algorithm.get_statistics().items():
                logz.log_tabular(k, v)
            for k, v in replay_pool.get_statistics().items():
